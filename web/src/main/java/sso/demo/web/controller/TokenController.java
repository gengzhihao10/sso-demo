...
public class TokenController implements TokenApiService {
    ...
    @Override
    public SSOResponse<ResponseCodeEnum> checkToken(HttpServletRequest request) {
        ...
        if (ObjectUtil.isNull(token)){
            ...
            return SSOResponse.fail(ResponseCodeEnum.TOKEN_IS_NULL);
        }
        //传入用户名不能为空
        if (ObjectUtil.isNull(userName)){
            log.error("传入用户名为空,token为 {}",token);
            return SSOResponse.fail(ResponseCodeEnum.TOKEN_USERNAME_IS_NULL);
        }
        //解析用户名
        ...
        try {
            ...
            return SSOResponse.fail(ResponseCodeEnum.TOKEN_USERNAME_PARSE_FAILED);
        }
