...
public class TokenService {
    ...
    public SSOResponse<TokenCommandOutput> generateToken(RestTokenCommand restTokenCommand){
        ...
        if (CollectionUtils.isEmpty(userDOS)){
            ...
            return SSOResponse.fail(ResponseCodeEnum.USER_IS_NOT_EXIST);
        }
        //查出多个用户，说明数据有问题
        if (userDOS.size() > 1){
            log.error("查出用户为多个，无法获取token，用户名为 {}",restTokenCommand.getUsername());
            return  SSOResponse.fail(ResponseCodeEnum.USER_DATA_WRONG);
        }
    ...
    public SSOResponse<ResponseCodeEnum> checkToken(String token, String userName){
        Claims claims = jwtUtil.parseToken(token);
        //判断是否过期
        if (ObjectUtil.isNull(claims) || jwtUtil.judgeTokenExpiration(claims.getExpiration())){
            log.info("token已过期，token为 {}",token);
            return SSOResponse.fail(ResponseCodeEnum.TOKEN_OUT_OF_DATE);
        }
        //校验用户是否存在
        ...
        if (ObjectUtil.isNull(userDO)){
            ...
            return SSOResponse.fail(ResponseCodeEnum.TOKEN_USER_NOT_EXIST);
        }
